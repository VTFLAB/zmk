...
static int event_mapper(const zmk_event_t *eh, zmk_studio_Notification *n) {
    ...
    // roBa では MOUSE (Layer 4) は automouse-layer、SCROLL (Layer 5) は
    // scroll-layers で trackball 動作中に高頻度トグルする。これを通知すると
    // BLE indication が殺到して BT スタックを飽和させ HID 出力に支障が出る
    // (2026-05-05 にトラックボール操作不能事象あり)。filter で除外する。
    if (layer_ev->layer == 4 || layer_ev->layer == 5) {
        return -ENOTSUP;
    }
